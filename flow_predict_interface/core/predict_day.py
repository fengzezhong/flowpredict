...
def pre_predict_data(train_data, scaler):
    ...
    train_flow = flows[:n_hour - 19]
    test_flow = flows[n_hour - 19:]
...
def predict_oneday(train_data, city, model, scaler):
    ...
    for i in range(1, 8):
        predictday = (last_time + datetime.timedelta(days=+i)).strftime("%Y%m%d%H%M")
...
def file_handle_and_predict_day(file_path, work_id, type, is_train):
    ...
    def file_handle_and_predict_day_asyn(file_path, work_id, type, is_train):
    ...
    _thread.start_new_thread(file_handle_and_predict_day_asyn, (file_path, work_id, type, is_train))
