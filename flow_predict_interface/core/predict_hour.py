...
def pre_predict_data(train_data, scaler):
    ...
    train_flow = flows[:n_hour - 36]
    test_flow = flows[n_hour - 36:]
...
def predict_oneday(train_data, city, model, scaler):
    ...
    for i in range(1, 25):
        predictday = (last_time + datetime.timedelta(hours=+i)).strftime("%Y%m%d%H%M")
...
def file_handle_and_predict_hour(file_path, work_id, type, is_train):
    ...
    def file_handle_and_predict_hour_asyn(file_path, work_id, type, is_train):
        ...
        city_process = round((89.19 / city_len), 2)
    ...
    _thread.start_new_thread(file_handle_and_predict_hour_asyn, (file_path, work_id, type, is_train))
